...
var _idKho = 'cfe3b13c-b614-4225-93d8-17f155620975';
...
function confirmPopup(title, warning, handlerYes, handlerNo, option, style, textYes, textNo) {
    ...
    sBuild += `<span style="margin-left:5px;"><a onclick="confirmNo();" href="javascript:void(0)" id="btnNO" class="btn btn-default" tabindex="1">${textNo || 'Không'}</a></span>`;
    sBuild += '</div></div>';
    $('body').append(sBuild);
...
$(document).ready(function () {
    ...
    $('a.scrollgotop').on('click', function (event) {
        $('html, body').animate({
            scrollTop: 0
        }, 1000, "easeInOutExpo");
...
function ConvertDateToTuoiKB(ngaySinh, homNay) {
    // Yêu cầu ngaySinh, homNay có định dạng: 'MM/DD/YYYY'
    var datNgaySinh = new Date(ngaySinh);
    var datHomNay = new Date(homNay);
    var year = datHomNay.getFullYear() - datNgaySinh.getFullYear();
...
const KyTu = ["aeouidy", "áàạảãâấầậẩẫăắằặẳẵ", "éèẹẻẽêếềệểễ",
...
function addTHeaderTable(idTable, srtTHeader) {
    srtTHeader = srtTHeader || "";
    var myTable = jQuery(idTable);
    ...
    jQuery(srtTHeader).appendTo(thead);
}

/**
 * Hàm tính tổng cộng tiền trên bảng dữ liệu và html.
 * @param {String} idTable id bảng html
 * @param {Array} myList Mảng dữ liệu
 * @param {Number} CountColumn Số cột bảng hiển thị
 * @param {Number} sumColIndex Index cột sẽ tính tổng tiền
 * @param {Object} options {
 * isRowSum: có hiện row tổng không
 * sumColIndex: Index cột sẽ tính tổng tiền
 * sumTextColIndex Index cột hiển thị text cộng tổng
 * sumText text hiển thị ở cột đã chỉ định
 * }
 */
function buildHtmlTable(idTable, myList, CountColumn, options) {
    var myTable = $(idTable);
    ...
    for (var i = 0; i < myList.length; i++) {
        let row$ = $('<tr/>');
        let colspanIndex = -1;
        if (myList[i]["ColSpanIndex"] != null) colspanIndex = myList[i]["ColSpanIndex"];
        let colspan = 1;
        if (myList[i]["ColSpan"] != null) colspan = myList[i]["ColSpan"];
        for (var colIndex = 0; colIndex < CountColumn; colIndex++) {
            var cellValue = myList[i][columnSet[colIndex]] || "";
            let colEl = $('<td/>');
            if (typeof cellValue == "number") {
                // tính tổng cột colIndex
                if (typeof sumCols[colIndex] === 'undefined') {
                    sumCols[colIndex] = 0;
                }
                sumCols[colIndex] += cellValue;
                // format và hiển thị html
                cellValue = ConverTienCoDauPhay2(cellValue);
                colEl.addClass("text-right")
            } else if (strIsX(cellValue)) {
                if (typeof sumCols[colIndex] === 'undefined') {
                    sumCols[colIndex] = 0;
                }
                sumCols[colIndex] += 1;
                colEl.addClass("text-center");
            } else if (typeof cellValue == "boolean") {
                cellValue = cellValue ? "Có" : "Không";
                colEl.addClass("text-center")
            } else if (strIsDateTime(cellValue)) {
                cellValue = moment(cellValue).format(_options.isDateTime ? "DD/MM/YYYY" : "HH:mm DD/MM/YYYY");
                colEl.addClass("text-center")
            }
            if (colIndex == colspanIndex) {
                colEl.attr("colspan", colspan);
                colIndex = colIndex + colspan - 1;
            }
            row$.append(colEl.html(cellValue));
        }
        tbody.append(row$);
    }

    if (_options.isRowSum) {
        let row$ = $('<tr style="font-weight:bold"/>');
        sumTotal = _.reduce(sumCols, (a, b) => a + b, 0);
        sumCols[_options.sumColIndex] = sumTotal;
        for (var colIndex = 0; colIndex < CountColumn; colIndex++) {
            if (colIndex == _options.sumTextColIndex) {
                row$.append($('<td class="text-right"/>').html(_options.sumText));
            } else if (typeof sumCols[colIndex] !== 'undefined') {
                // format và hiển thị html
                var cellValue = ConverTienCoDauPhay2(sumCols[colIndex], 2);
                row$.append($('<td class="text-right"></td>').html(cellValue));
            } else {
                row$.append($('<td/>').html(''));
            }
        }
