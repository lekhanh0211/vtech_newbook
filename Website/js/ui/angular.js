    ...
    if (comparatorType !== 'function') {
      if (comparatorType === 'boolean' && comparator) {
        ...
        comparator = function(obj, text) {
          ...
          return locdau((''+obj).toLowerCase()).indexOf(text) > -1;
        };
      }
    }
          ...
          (function(path) {
              ...
              if (typeof expression[path] === 'string') {
                  predicates.push(function (value) {
                      return search(path == '$' ? value : (value && value[path]), locdau(expression[path]));
                  });
              }
              else
              {
...
function currencyFilter($locale) {
  ...
  return function(amount, currencySymbol, fractionSize, fractionSizeMin) {
    ...
    return (amount == null)
        ? amount
        : formatNumber(amount, formats.PATTERNS[1], formats.GROUP_SEP, formats.DECIMAL_SEP, fractionSize, fractionSizeMin).
...
function numberFilter($locale) {
  var formats = $locale.NUMBER_FORMATS;
  return function (number, fractionSize, fractionSizeMin) {

    // if null or undefined pass it through
    return (number == null)
        ? number
        : formatNumber(number, formats.PATTERNS[0], formats.GROUP_SEP, formats.DECIMAL_SEP,
                       fractionSize, fractionSizeMin);
  };
}

var DECIMAL_SEP = '.';
function formatNumber(number, pattern, groupSep, decimalSep, fractionSize, fractionSizeMin) {
  ...
  if (!hasExponent) {
    ...
    if (isUndefined(fractionSizeMin)) {
        fractionSizeMin = 0;
    }
    ...
    while (fraction.length < fractionSizeMin) {
      fraction += '0';
    }

    if (fractionSize && fractionSize !== "0" && fraction.length) formatedText += decimalSep + fraction.substr(0, fractionSize);
  } else {
